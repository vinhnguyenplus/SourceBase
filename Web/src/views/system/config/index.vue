	...
	<div class="sys-config-container">
		...
		<el-card class="full-table" shadow="hover" style="margin-top: 5px">
			<Table ref="tableRef" v-bind="tb.tableData" :getData="getData" :exportChangeData="exportChangeData" @sortHeader="onSortHeader" @selectionChange="tableSelection" border>
				<template #command>
					<el-button type="primary" icon="ele-Plus" @click="openAddConfig" v-auth="'sysConfig:add'"> Add New </el-button>

					<el-button v-if="state.selectlist.length > 0" type="danger" icon="ele-Delete" @click="bacthDelete" v-auth="'sysConfig:batchDelete'"> Batch delete </el-button>
				</template>
				...
				<template #action="scope">
					<el-button icon="ele-Edit" size="small" text type="primary" @click="openEditConfig(scope.row)" v-auth="'sysConfig:update'"> Edit </el-button>
					<el-button icon="ele-Delete" size="small" text type="danger" @click="delConfig(scope.row)" v-auth="'sysConfig:delete'" :disabled="scope.row.sysFlag === 1"> Delete </el-button>
				</template>
			</Table>
		</el-card>
...
const tb = reactive<TableDemoState>({
	tableData: {
		...
		config: {
			...
			showSelection: auth('sysConfig:batchDelete'), // Whether to display table multi-select
            ...
            printName: 'System Parameters', // Print preview title name
		},
